...
// index表示的是第几个块
// 由于采用了一位表示两个块的方法
// 相邻兄弟对应的标志位是一样的,都是在编号为(index/2)/8的数组中
// 移位后异或表示将对应的位数翻转
...
// 只有当自己可分配、伙伴不可分配或者自己不可分配、伙伴可分配时，才能添加到free列表中
// 如果两个都是可分配的，说明应该合并成为一个更大的块，此时标记会有重复
// 如果两个都是不可分配的，说明没有空闲的空间
int
